...
interface DetailProposalContextData {
  ...
  error?: any;
  fetch: (idProposal: string, startBlock: string) => Promise<void>;
}
...
export function DetailProposalProvider({ children }: BaseContextProps) {
  ...
  async function fetchData(idProposal: string, startBlock: string) {
    const [_x, currentblock] = await Promise.all([
      getContentAndHistoryProposal(idProposal),
  ...
  async function getUserInforAndVotingResult(idProposal: string, startBlock: string) {
    try {
              ...
              calls: [
                {
                  ...
                  params: [address || '', idProposal, startBlock],
                },
              ],
            },
            {
  ...
  async function getContentAndHistoryProposal(idProposal: string) {
    ...
    try {
      const response = await dashboardClient.get('/proposals/' + idProposal);
  ...
  return (
    <DetailProposalContext.Provider
      value={{
        ...
        data,
        error,
        fetch: (idProposal, startBlock) => fetchData(idProposal, startBlock),
      }}
    >
      {children}
    </DetailProposalContext.Provider>
